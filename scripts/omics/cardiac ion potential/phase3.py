...
PHASE_TO_PLOT = "Phase 3"
OUTPUT_PDF = "Normal_vs_HF_Phase3_mirrored_DESeq2_heatmap.pdf"
...
GENE_TO_ENSEMBL = {
    ...
    "KCNE2": "ENSG00000159197",
    "KCNE3": "ENSG00000175538",
    "KCNE4": "ENSG00000152049",
    "KCNE5": "ENSG00000176076",
    "KCNQ1": "ENSG00000053918",
    "KCNE1": "ENSG00000180509",
    "KCNJ12": "ENSG00000184185",
    "KCNJ14": "ENSG00000182324",
    "KCNJ2": "ENSG00000123700",
    "KCNJ4": "ENSG00000168135"
}
...
def main():
    ...
    if phase_df.empty:
        raise ValueError(
            f"No genes found for {PHASE_TO_PLOT}"
        )
    ...
    phase_df["gene_id"] = phase_df["gene"].map(
        GENE_TO_ENSEMBL
    )
    ...
    if unmapped_genes:
        print(
            "Warning: No Ensembl mapping for: "
            + ", ".join(unmapped_genes)
        )
    ...
    deseq_df = deseq_df.drop_duplicates(
        subset="gene_id",
        keep="first"
    )
    ...
    if missing_deseq:
        print(
            "Warning: No DESeq2 value found for: "
            + ", ".join(missing_deseq)
        )
    ...
    if df.empty:
        raise ValueError(
            "No usable DESeq2 values found for Phase 3."
        )

    hf_values = df["log2FoldChange"].to_numpy(
        dtype=float
    )
    ...
    row_order, row_tree = cluster_rows(
        matrix,
        hf_values
    )
    ...
    title_ax.text(
        0.5,
        0.70,
        "Phase 3 Differential-Expression Heatmap: Normal vs Heart Failure",
        ha="center",
        va="center",
        fontsize=15,
        fontweight="bold"
    ...
    title_ax.text(
        0.5,
        0.20,
        "Heart Failure = DESeq2 log2FC; Normal = inverse of log2FC",
        ha="center",
        va="center",
        fontsize=9
    )
    ...
    fig.text(
        0.60,
        0.018,
        "Heart Failure = DESeq2 log2FC (HF / Normal). "
        "Normal = inverse of that value. "
